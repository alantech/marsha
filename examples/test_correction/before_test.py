...
class TestExtractConnectionInfo(unittest.TestCase):
    ...
    def test_basic_mysql(self):
        expected = '{"protocol": "jdbc:mysql", "dbUser": "user", "dbPassword": "pass", "host": "0.0.0.0", "port": 3306, "database": "mydb"}'
        result = extract_connection_info('jdbc:mysql://0.0.0.0:3306/mydb?user=user&password=pass')
        self.assertEqual(result, expected)

    def test_mysql_with_extra(self):
        expected = '{"protocol": "jdbc:mysql", "dbUser": "user", "dbPassword": "pass", "host": "0.0.0.0", "port": 3306, "database": "mydb", "extra": {"ssl": "require"}}'
