...
def test_normalize_argv_maps_bare_form_to_compile():
...
def test_normalize_argv_leaves_subcommands_and_help_alone():
...
def test_print_help_overview_lists_subcommands(capsys):
...
def test_print_help_compile_shows_compile_usage(capsys):
...
def test_print_help_unknown_topic_reports_and_lists(capsys):
...
def test_run_help_topic_returns_zero(capsys):
    assert base.run(['help', 'compile']) == 0
    assert 'usage: marsha compile' in capsys.readouterr().out


def test_run_no_command_prints_help_and_returns_two(capsys):
    assert base.run([]) == 2
    assert 'compile' in capsys.readouterr().err


def test_run_dispatches_compile(monkeypatch):
    calls = []
...
def test_run_bare_alias_warns_and_still_compiles(monkeypatch, capsys):
    calls = []
    monkeypatch.setattr(base, '_setup_runtime',
                        lambda args: calls.append('setup'))
