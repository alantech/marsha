...
from marsha.context import ModelDescriptor


def _m(name: str, ctx: int | None = None) -> ModelDescriptor:
...
def test_rank_standard_smallest_fitting() -> None:
...
def test_rank_standard_target_follows_configured_model() -> None:
...
def test_rank_standard_no_current_defaults_to_default_window() -> None:
    models = [_m('small', 200000), _m('big', 400000)]
    assert model_match.rank_models(models, 'model') == 'small'


def test_rank_standard_explicit_target_overrides() -> None:
    models = [_m('tiny', 8192), _m('mid', 131072), _m('big', 262144)]
    assert model_match.rank_models(
        models, 'model', profile=_profile(target_context=65536)) == 'mid'


def test_rank_standard_exact_target_fits() -> None:
    models = [_m('just-below', 65535), _m('exact', 65536)]
    assert model_match.rank_models(
        models, 'model', profile=_profile(target_context=65536)) == 'exact'


def test_rank_standard_best_effort_largest_when_none_fit() -> None:
    models = [_m('tiny', 8192), _m('mid', 32768)]
    assert model_match.rank_models(
        models, 'model', current='gpt-5-mini') == 'mid'


def test_rank_standard_unknown_context_sorts_last() -> None:
    models = [_m('mystery'), _m('mid', 400000)]
    assert model_match.rank_models(
        models, 'model', current='gpt-5-mini') == 'mid'


def test_rank_strong_largest() -> None:
    models = [_m('tiny', 8192), _m('mid', 131072), _m('big', 262144)]
    assert model_match.rank_models(models, 'model_strong') == 'big'


def test_rank_strong_unknown_context_sorts_last() -> None:
    models = [_m('mystery'), _m('big', 262144)]
    assert model_match.rank_models(models, 'model_strong') == 'big'


def test_rank_no_signal_falls_back_to_list_order() -> None:
...
def test_rank_price_breaks_context_ties() -> None:
...
def test_rank_profile_override_drives_role() -> None:
    models = [_m('tiny', 8192), _m('big', 262144)]
    strong: Any = dict(model_match.ROLE_PROFILES['model_strong'])
    assert model_match.rank_models(models, 'model', profile=strong) == 'big'


def test_rank_empty_or_unknown_role() -> None:
...
def test_apply_remaps_each_role_to_its_closest_match() -> None:
...
def test_apply_no_fitting_model_both_roles_take_largest() -> None:
...
def test_apply_single_model_remapped_to_it() -> None:
...
def test_apply_keeps_models_that_are_served() -> None:
...
def test_apply_keeps_served_standard_and_ranks_strong() -> None:
...
def test_apply_empty_or_none_is_a_noop() -> None:
...
def test_apply_pinned_model_is_kept_and_flagged() -> None:
...
def test_apply_pinned_models_in_list_are_silent() -> None:
...
def test_apply_pinned_via_config_file_is_kept() -> None:
