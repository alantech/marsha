...
Do NOT report a performance or micro-optimization suggestion (hoisting, caching, batching, parallelizing, or a complexity claim) unless it is in a hot loop or works on data large enough to matter; a one-off call or a single-digit-sized collection is not a performance problem.
